...
if ($_POST['bn_save'] || $_POST['bn_xmit']) {
    ...
    for ($i = 0; isset($_POST['form_proc_type'][$i]); ++$i) {
        ...
        $poseq = sqlInsert(
            ...
            "procedure_order_seq = ? ",
            array($formid, $_POST['form_proc_type_diag'][$i], $_POST['form_proc_eye'][$i], $_POST['form_proc_dosis'][$i], $_POST['form_proc_order_title'][$i], $ptid, $ptid, $procedure_order_seq['increment'])
        );
        sqlCommitTrans();
...
function addProcLine(flag = false) {
    ...
    let cell = "<table class='table table-condensed proc-table'><tr><td onclick='deleteRow(event)' class='itemDelete'><i class='fa fa-remove'></i></td>" +
        ...
        "<td><select class='form-control' name='form_proc_eye[" + i + "]' " +
        "title='<?php echo xla('Click to add a eye'); ?>' onfocus='this.blur()' style='cursor:pointer;cursor:hand'>" +
        "<option value='OD'>OD</option>" +
        "<option value='OI'>OI</option>" +
        "<option value='AO'>AO</option>" +
        "</select></td>" +
        "<td><select class='form-control' name='form_proc_dosis[" + i + "]' " +
        "title='<?php echo xla('Click para agregar dosis'); ?>' onfocus='this.blur()' style='cursor:pointer;cursor:hand'>" +
        "<option value=''> </option>" +
            ...
            <form class="form-horizontal" method="post" action="<?php echo $rootdir ?>/forms/procedure_order/new.php?id=<?php echo attr_url($formid); ?>" onsubmit="return validate(this)">
                ...
                <fieldset>
                    ...
                    <div class="col-xs-12">
                        ...
                        <div class="form-group">
                            <label for="form_data_ordered" class="control-label col-sm-3 oe-text-to-right"><?php echo xlt('Especialidad'); ?></label>
                            <div class="col-sm-2">
                                <?php
                                $historyOrderOpts = array(
                                    'data_type' => 1,
                                    'field_id' => 'history_order',
                                    'list_id' => 'physician_type'
                                );
                    ...
                    <div class="col-md-12 procedure-order-container table-responsive">
                        ...
                        if ($formid) {
                            $opres = sqlStatement(
                                ...
                                "pc.diagnoses, pc.ojo, pc.dosis, pc.procedure_order_title, " .
                        ...
                        foreach ($oparr as $oprow) {
                            ...
                            <table class="table table-condensed proc-table" id="procedures_item_<?php echo (string) attr($i) ?>">
                                ...
                                <tr>
                                    ...
                                    <th><?php echo xlt("Ojo");?></th>
                                    <th><?php echo xlt("Dosis");?></th>
                                </tr>
                                </thead>
                                <tbody>
                            <tr>
                                ...
                                <td>
                                    <select class='form-control' name='form_proc_eye[<?php echo attr($i); ?>]'
                                            title='<?php echo xla('Click to add a eye'); ?>'
                                            style='cursor:pointer;cursor:hand'>
                                        <option selected='selected'><?php echo attr($oprow['ojo']) ?></option>
                                        <option value="OD">OD</option>
                                        <option value="OI">OI</option>
                                        <option value="AO">AO</option>
                                    </select>
                                ...
                                <td>
                                    <select class='form-control' name='form_proc_dosis[<?php echo attr($i); ?>]'
                                            title='<?php echo xla('Click para agregar dosis'); ?>'
                                            style='cursor:pointer;cursor:hand'>
                                        <option selected='selected'><?php echo attr($oprow['dosis']) ?></option>
                                        <option value="1era">1era</option>
                                        <option value="2da">2da</option>
                                        <option value="3era">3ra</option>
                                        <option value="4ta">4ta</option>
                                        <option value="5ta">5ta</option>
                                        <option value="6ta">6ta</option>
                                    </select>
