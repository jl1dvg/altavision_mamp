...
if ($_POST['statustype'] !='') {
    ...
    if (!empty($tkpid)) {
        ...
        if ($GLOBALS['auto_create_new_encounters'] && $apptdate == date('Y-m-d') && (is_checkin($status) == '1') && !$is_tracker) {
            ...
            $encounter = todaysEncounterCheck($tkpid, $apptdate, $genenc['reason'], $genenc['cirugia'], $genenc['facility'], $genenc['billing_facility'], $genenc['provider'], $genenc['category'], false);
