...
// Check authorization.
$thisauth = acl_check('patients', 'labcontrol');
if (!$thisauth) {
...
function openResults(orderid,pid) {
    top.restoreSession();
...
// patients notes it will wipe out this script's list. We need 3 viewports.
    window.open('single_order_results.php?orderid=' + encodeURIComponent(orderid) + '&pid=' + encodeURIComponent(pid), '_blank', 'toolbar=0,location=0,menubar=0,scrollbars=yes');
//
...
// Generate order columns.
            if ($lastpoid != $order_id) {
                ...
                echo "/interface/orders/orders_results_express.php?orderid=";
