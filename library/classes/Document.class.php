...
class Document extends ORDataObject
{
    ...
    */
    ...
    function getPatientData2($pid)
    {
        $sql = "select pubpid from patient_data where pid=? order by date DESC limit 0,1";
    ...
    function createDocument(
        ...
        if ($this->storagemethod == 1) {
            ...
            if ((!empty($higher_level_path)) && (is_numeric($patient_id) && $patient_id > 0)) {
                ...
                $sql = "select pubpid from patient_data where pid=? order by date DESC limit 0,1";
                $sqlcat = "select name from categories where id=?";
                $titleres = sqlQuery($sql, array($patient_id));
                $cat = sqlQuery($sqlcat, array($category_id));
                $filepath = $repository . text($titleres['pubpid']) . '/' . $_SESSION['encounter'] . '/' . text($cat['name']) . '/';
                $path_depth = 1;
            }
        ...
        $this->encounter_id = $encounter_id ? $encounter_id : $_SESSION['encounter'];
