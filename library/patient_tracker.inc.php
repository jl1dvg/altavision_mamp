...
function manage_tracker_status($apptdate, $appttime, $eid, $pid, $user, $status = '', $room = '', $docnotes = '', $enc_id = '')
{
    ...
    $pc_appt = sqlQuery("SELECT `pc_apptstatus`, `pc_room`, `pc_docnotes` FROM `openemr_postcalendar_events` WHERE `pc_eid` = ?", array($eid));
    ...
    if ($docnotes != $pc_appt['pc_docnotes']){
        sqlStatement("UPDATE `openemr_postcalendar_events` SET `pc_docnotes` = ? WHERE `pc_eid` = ?", array($docnotes, $eid));
    }

    if ($docnotes != $pc_appt['pc_docnotes']){
        sqlStatement("UPDATE `patient_tracker` SET `docnotes` = ? WHERE `eid` = ?", array($docnotes, $eid));
    }
