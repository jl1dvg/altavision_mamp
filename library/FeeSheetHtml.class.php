...
class FeeSheetHtml extends FeeSheet
{
  ...
  //
    ...
    public static function genProviderSelect2($tagname, $toptext, $default, $disabled = false)
    {
        $s = "   <select name='" . attr($tagname) . "'";
        if ($disabled) {
            $s .= " disabled";
  ...
  //
    ...
    public function genPriceLevelSelect2($tagname, $toptext, $default = 0, $disabled = false)
    {
        // echo "<!-- pr_id = '$pr_id', pr_selector = '$pr_selector' -->\n"; // debugging
        $s = "   <select name='" . attr($tagname) . "'";
        if ($disabled) {
        ...
        $s .= ">";
        $s .= self::generatePriceLevelOptionList($toptext, $default);
        $s .= "</select>\n";
        return $s;
    }
  ...
  // Generate a price level drop-down defaulting to the patient's current price level.
  //
    public function generatePriceLevelOptionList($toptext, $default = 0)
    {
        $s = '';
